...
аллоцирует kacho-vpc, **internal-only** — только на internal-проекции Network / через
`InternalNetworkService.GetNetwork`; в публичном Network API его нет).

В скоупе:
- 7 verbatim-ресурсов с CRUD + Move + ListOperations.
- NetworkInterface: CRUD + ListOperations + `AttachToInstance` / `DetachFromInstance`
  (выставляют/очищают `used_by`); ссылка на `Address`-ресурсы по id (`v4_address_ids`/`v6_address_ids`).
...
- Internal endpoints (на порту 9091, не выставляется на external TLS endpoint;
  ...
  `InternalZoneService`, `InternalNetworkService` (Network + internal `vpn_id`),
  `InternalNetworkInterfaceService` (`GetNetworkInterface` / `ListByHypervisor` → `InternalNetworkInterface`
  с data-plane-инфой; `ReportNiDataplane` — write-back data-plane-state от `kacho-vpc-implement`, эпик `KAC-2`),
  `InternalCloudService` (IPAM/admin).
...
### 2.1 NetworkInterface (first-class ресурс, эпик `KAC-2`)

`NetworkInterface` (NIC) — first-class AWS-ENI-подобный ресурс домена VPC (вариант А: NIC живёт
в `kacho-vpc`, не в `kacho-vpc-implement`). Folder-level (`folder_id` обязателен), принадлежит
`Subnet` (`subnet_id`; FK ON DELETE RESTRICT — миграция 0012 откатила KAC-31's CASCADE: NIC
жёстко блокирует свою подсеть; удаляем снизу вверх NIC → Address → Subnet → Network). Ссылается на
`Address`-ресурсы **по id**: `v4_address_ids[]` /
...
`DetachFromInstance` (flat-колонки `used_by_type`/`used_by_id`/`used_by_name` на `network_interfaces`).
`status` — грубый enum (`PROVISIONING`/`ACTIVE`/`AVAILABLE`/`FAILED`/`DELETING`).
...
- **`InternalNetworkInterface` (internal-port 9091, `InternalNetworkInterfaceService`):** +
  ...
  (`169.254.x.y`), `container_id`, резолвленные IP-строки (`v4_addresses`/`v6_addresses`),
  резолвленный `vpn_id`, `status_error`, `revision`. **Никогда не на публичной поверхности.**

Миграции (исторические — все свёрнуты в `0001_initial.sql` после squash
2026-05-17): NIC schema (KAC-2/KAC-7), FK NIC→Subnet ON DELETE RESTRICT
(KAC-33, после отката KAC-31's CASCADE), generated-колонка
`addresses.internal_subnet_id` из v4 ИЛИ v6 subnet_id (KAC-34), MAC-address
с cloud-wide UNIQUE (KAC-48), CHECK `jsonb_array_length(v4_address_ids)<=1`
+ симметрично v6 (KAC-55, DB-level cardinality ≤1 v4 / ≤1 v6 на NIC),
auto-association RouteTable ↔ Subnet через PL/pgSQL триггеры (KAC-56:
AFTER INSERT ON route_tables auto-assoc'ит Subnet'ы сети с `route_table_id
IS NULL`; BEFORE INSERT ON subnets подставляет самую раннюю RT этой сети;
FK ON DELETE SET NULL обнуляет `subnet.route_table_id` при RT.Delete;
AFTER UPDATE OF route_table_id эмитит `Subnet.UPDATED` в `vpc_outbox` с
маркером `auto_association: true`). Network → default SG **по-прежнему**
inline в `network.go::doCreate` (id-generation crockford-base32 не
реализуется в PL/pgSQL без портирования `kacho-corelib/ids`).
...
- Subnet → Address (internal): RESTRICT — FK `addresses_internal_subnet_fkey` через generated-колонку `addresses.internal_subnet_id`, которая с миграции `0013_address_internal_subnet_id_v6.sql` (KAC-34) выводится из `internal_ipv4->>'subnet_id'` **ИЛИ** `internal_ipv6->>'subnet_id'` — т.е. и v4-, и v6-internal-адрес блокируют удаление своей подсети (раньше — только v4). Sync-precheck в `SubnetService.Delete` (`AddressesBySubnet`) тоже покрывает обе семьи.
- Subnet → NetworkInterface: RESTRICT (`network_interfaces.subnet_id` ON DELETE RESTRICT — миграция `0012_nic_subnet_restrict.sql`, KAC-33, откатила KAC-31's `0011_nic_subnet_cascade.sql`). NIC блокирует удаление своей подсети — и транзитивно сети; сначала удалите NIC'и (`SubnetService.Delete` отдаёт sync `FailedPrecondition` со списком NIC-id; FK RESTRICT в worker'е — атомарный backstop). Address в использовании у NIC тоже нельзя удалить (`AddressService.Delete` → `FailedPrecondition`, см. §2.1 / `Address.used`). Порядок удаления — снизу вверх: NIC → Address → Subnet → Network.
...
| NetworkInterface | `ids.PrefixSubnet` (переиспользуется) | `e9b` | `e9b + ...` (см. `network_interface.go::niResourceID` / `Create`) |
...
Subnet/Address/NetworkInterface — `e9b` (NIC переиспользует `PrefixSubnet`). (Region/Zone — это строки вида `ru-central1`/`ru-central1-a`, не prefix-id.)
...
- Repo Insert/Update — FK violations, EXCLUDE constraint (CIDR overlap),
  UNIQUE violation `(folder_id, name)` для всех ресурсов (`networks_folder_id_name_key`
  non-partial; для остальных — partial `WHERE name <> ''` — inline в baseline
  `0001_initial.sql`)
...
2. **DB-level** (baseline `0001_initial.sql`): `EXCLUDE USING gist` constraint
...
(`migrations.go`). Физически **один файл** — `0001_initial.sql` (clean
baseline после squash 34 миграций, ветка `chore/squash-migrations`,
2026-05-17):

- `0001_initial.sql` — единая squashed baseline. Все таблицы (`operations`,
  `networks`, `subnets`, `addresses`, `address_references`, `route_tables`,
  `security_groups`, `gateways`, `private_endpoints`, `network_interfaces`,
  `vpc_outbox`, `vpc_watch_cursors`, `address_pools`,
  `address_pool_address_override`, `address_pool_network_default`,
  `cloud_pool_selector`, `address_pool_free_ips`, `ipv6_pool_cursors`,
  `ipv6_allocated_ips`, `ipv6_released_offsets`) создаются сразу в схеме
  `kacho_vpc` со всеми CHECK / FK / UNIQUE / EXCLUDE / generated columns /
  индексами / триггерами inline (skill `evgeniy` §5 E.5). Helper-функции
  (`kacho_labels_valid`, `vpc_outbox_notify`, `rt_auto_assoc_subnets`,
  `subnet_auto_pick_rt`, `subnets_outbox_emit_route_table_change`) тоже
  inline. Нет legacy-полей (`vpn_id`, NIC data-plane — drop'нуты эпиком
  `KAC-79/KAC-36`). Id-колонки — `TEXT`.
  `networks_folder_id_name_key` — non-partial UNIQUE `(folder_id, name)`;
  остальные 7 пользовательских ресурсов — partial UNIQUE `WHERE name <> ''`.
...
**Запреты:**
- НЕ редактировать `0001_initial.sql` после первого rollout в production
  (запрет #5 из workspace CLAUDE.md). Текущая allowance на правку — только
  в ветке `chore/squash-migrations` и до первого применения на kind-стенде.
- Новая миграция = новый файл с инкрементным номером (следующий — `0002_*`).
- `make sync-migrations` теперь **no-op**: общая `operations`-таблица из
  `kacho-corelib/migrations/common/0001_operations.sql` встроена в baseline
  (под `kacho_vpc.operations`), повторное копирование общего файла создало
  бы конфликтующий unqualified `public.operations`.

**Schema = `kacho_vpc`** (skill evgeniy §5 E.4): все VPC-таблицы (20),
5 user-функций (`kacho_labels_valid` + 4 trigger-функции) и
`goose_db_version` живут в схеме `kacho_vpc` — НЕ в `public`. `btree_gist` extension
остаётся в `public` (extension-owned). Каждое соединение должно установить
`search_path TO kacho_vpc, public` — это сделано через libpq-параметр
`options=-c search_path=kacho_vpc,public`, который `config.baseDSN()` добавляет
автоматически к `cfg.DSN()` / `cfg.MigrateDSN()` / `cfg.SlaveDSN()`. Тесты,
которые строят DSN из testcontainers напрямую (мимо config), используют
`appendSearchPathOptions` helper (см. `internal/repo/integration_test.go`).
Триггерные функции и `CHECK kacho_labels_valid(labels)` вызываются по
unqualified-имени — резолвятся через тот же search_path.

⚠️ **BREAKING CHANGE (greenfield)**: squash drop'нул историю миграций
0002..0034. Существующие БД (kind dev-стенд, любая локальная) — должны быть
полностью пересозданы (`make ci-down -v && make ci-up` в `kacho-deploy`, либо
delete pvc + helm re-install). Это dev-only — production-релизов VPC ещё не было.
...
- **AddressPool** — **глобальный
  ...
  - Поля: `id`, `name`, `description`, `labels`, **`v4_cidr_blocks`** +
    **`v6_cidr_blocks`** (KAC-71: разделено по family вместо общего
    `cidr_blocks` — pool явно v4-only / v6-only / dual-stack;
    backfill миграцией `0022_addresspool_split_cidr_family.sql`, см. §16.2),
    `kind` (EXTERNAL_PUBLIC; EXTERNAL_TEST/RESERVED_INTERNAL — `reserved` в
    proto enum), `zone_id` (TEXT без FK, см. KAC-15 Geography в compute),
    `is_default`, **`selector_labels`**, **`selector_priority`**.
  - Update мутирует CIDR-поля только если соответствующий `replace_v4_cidr_blocks`
    / `replace_v6_cidr_blocks` флаг = `true`. Body-array без флага игнорируется
    (REQ-IPL-UPD-05/06). Pool не может стать пустым (`cardinality(v4)+cardinality(v6) > 0`
    после Update — иначе `InvalidArgument`).
...
KAC-71 (family-aware filter): на каждом из 5 шагов pool пропускается, если его
запрошенный family-список пуст. Резолв для v4 берёт pool только если
`cardinality(v4_cidr_blocks) > 0`, симметрично для v6. Override / network_default
не «форсируют» family-mismatch — cascade проваливается дальше; ничего не
найдено → `ErrPoolNotResolved` (handler `ExplainResolution` ловит этот sentinel
и отдаёт HTTP 200 + `matched_via="none"`; `AllocateExternal*` оставляет
`FailedPrecondition`).

```
ВХОД: address_id, family ∈ {v4, v6}, zone_id, kind, network_selector?
HELPER: poolHasFamily(p, family) := family=v4 ? p.v4_cidr_blocks <> {} : p.v6_cidr_blocks <> {}

1. address_pool_address_override[address_id]                       (explicit per-address)
   → если resolved pool && poolHasFamily(p, family) → return p; иначе fall-through.
...
3. label-selector match через NetworkPoolSelector:
     SELECT id FROM address_pools
       WHERE selector_labels @> $network_selector       -- pool ⊇ network (containment)
         ...
         AND kind = $kind
         AND <v*_cidr_blocks NOT EMPTY>                 -- family filter
       ORDER BY
         (jsonb_object_size(selector_labels) - $network_size) ASC,  -- diff: точнее = выигрывает
         selector_priority DESC                                       -- explicit tie-break
       LIMIT 1;
4. zone_default        (is_default=true для zone+kind + family filter)
5. global_default      (is_default=true для zone IS NULL и kind + family filter)
```
