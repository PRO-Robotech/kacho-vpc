...
**Запреты:**
- НЕ редактировать применённые миграции (`0001_initial.sql` … `0012_nic_subnet_restrict.sql`) (запрет #5 из workspace CLAUDE.md).
- НЕ модифицировать `0001_operations.sql` (staging-копия из corelib).
- Новая миграция = новый файл с инкрементным номером (следующий — `0013_*`).
