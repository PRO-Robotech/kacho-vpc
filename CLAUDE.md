...
`Subnet` (`subnet_id`). Ссылается на `Address`-ресурсы **по id**: `v4_address_ids[]` /
`v6_address_ids[]` (один `Address` — максимум на одном NIC; enforced сервис-слоем через
`addresses.used` + referrer-tracking, как `address_references`). Несёт `security_group_ids[]`.
...
- Subnet → NetworkInterface: RESTRICT (`network_interfaces.subnet_id` ON DELETE RESTRICT)
