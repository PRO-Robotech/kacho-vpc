...
`Subnet` (`subnet_id`; FK ON DELETE CASCADE — миграция 0011, KAC-31, NIC не блокирует подсеть
напрямую). Ссылается на `Address`-ресурсы **по id**: `v4_address_ids[]` /
...
`0011_nic_subnet_cascade.sql` (NIC→Subnet FK: RESTRICT → CASCADE).
...
- Subnet → NetworkInterface: CASCADE (`network_interfaces.subnet_id` ON DELETE CASCADE — миграция `0011_nic_subnet_cascade.sql`, эпик KAC-31). NIC **не** блокирует свою подсеть напрямую: address-less NIC каскадно удаляется при удалении подсети; NIC **с** адресами всё ещё транзитивно блокирует подсеть, т.к. Address в использовании у NIC удалить нельзя (`AddressService.Delete` → `FailedPrecondition`, см. §2.1 / `Address.used`), а Subnet → Address — RESTRICT. NIC, приаттаченный к compute-инстансу, блокирует Subnet.Delete синхронно (`SubnetService.Delete` precondition-проверка).
