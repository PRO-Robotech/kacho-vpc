...
**Запреты:**
- НЕ редактировать применённые миграции (`0001_initial.sql` … `0013_address_internal_subnet_id_v6.sql`) (запрет #5 из workspace CLAUDE.md).
- НЕ модифицировать `0001_operations.sql` (staging-копия из corelib).
- Новая миграция = новый файл с инкрементным номером (следующий — `0014_*`).

**Schema = `kacho_vpc`** (skill evgeniy §5 E.4, миграция `0034_schema_rename_to_kacho_vpc.sql`):
все VPC-таблицы (21), 5 user-функций (`kacho_labels_valid` + 4 trigger-функции) и
