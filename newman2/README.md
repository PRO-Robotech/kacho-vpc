# newman2 — публичный API kacho-vpc, 100% coverage suite
...
> Существующий `newman/` остаётся как baseline. `newman2/` — отдельная
...
- **Conformance**: каждый кейс должен иметь зеркало против YC (через
  `--env yc` — не реализовано в newman2 v1, см. TEST-PLAN).
