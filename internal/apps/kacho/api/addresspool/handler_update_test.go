...
// parity).
//
// UpdateAddressPoolRequest несёт google.protobuf.FieldMask update_mask (прежние
...
// kachomock; остальные use-case'ы не нужны (тест зовёт Update + Get).
...
// seed кладёт pool в начальном состоянии (минуя use-case).
...
// getPB достаёт текущее состояние пула как proto через handler.Get.
...
// vpc8G-D7 — нет 2xx без применённого изменения (валидное поле + невалидное в одном mask).
