...
// остаётся tier `editor`. Top-level List остаётся `viewer` (visibility-фильтр —
...
// ресурсе): Check → `v_list`. Это НЕ top-level project-List (тот остаётся viewer).
...
// createChildRPCs — parent-scoped Create (Extract → project): остаётся tier
// `editor` на parent (F-7; create-self verb бессмыслен на ещё-несуществующем
...
// projectListRPCs — top-level project-scoped List (Extract → project): остаётся
// `viewer`. Visibility per-object идёт через iam ListObjects `viewer ∪ v_list`
