...
## Baseline: единственная миграция `0001_initial.sql` (greenfield)

Текущий state БД сформирован **одной** миграцией `0001_initial.sql` — clean
baseline после squash 34 исторических миграций (ветка `chore/squash-migrations`).
Стенд пересобирается с нуля, поэтому boilerplate-цепочка `0001..0034` свёрнута:

- Все таблицы создаются сразу в схеме `kacho_vpc` (без промежуточного rename
  из `public`, как делала старая `0034`).
- Все CHECK / FK / UNIQUE / EXCLUDE / generated columns / триггеры — **inline
  с `CREATE TABLE`** (skill `evgeniy` §5 E.5). Нет post-hoc
  `ALTER … ADD CONSTRAINT` в более поздних миграциях.
- Нет legacy-полей kube-ovn / netbox / `vpn_id` / NIC data-plane
  (drop'нуты эпиком `KAC-79/KAC-36`).
- Нет backfill-шагов / откатных миграций (`0011 → 0012` NIC→Subnet
  CASCADE/RESTRICT, `0016 → 0017` NIC `used_by` UNIQUE/drop — финальный
  state встроен сразу).
...
**Запрещено** (workspace-CLAUDE.md §запрет #5): редактирование
`0001_initial.sql` после первого rollout в production. Любая корректировка —
**новая миграция** `0002_<topic>.sql` с инкрементным номером. Текущая
allowance на правку baseline'а ограничена ветки `chore/squash-migrations` —
строго до того момента как новая ветка merge'нется в `main` и применится на
первом kind-стенде.
