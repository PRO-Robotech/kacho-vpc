...
func (r *AddressPoolRepo) List(ctx context.Context, f AddressPoolFilter, p Pagination) ([]*domain.AddressPool, string, error) {
...
func (r *AddressPoolRepo) Update(ctx context.Context, p *domain.AddressPool) (*domain.AddressPool, error) {
	...
	if tag.RowsAffected() == 0 {
		return nil, fmt.Errorf("%w: AddressPool %s", ErrNotFound, p.ID)
	}
...
func (r *AddressPoolRepo) Delete(ctx context.Context, id string) error {
	...
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("%w: AddressPool %s", ErrNotFound, id)
	}
...
func (r *AddressPoolRepo) ListAddressesByPool(ctx context.Context, poolID, folderFilter string, p Pagination) ([]*Address, string, error) {
	pageSize, err := validate.PageSize("page_size", p.PageSize)
	if err != nil {
		return nil, "", err
	}
...
func (r *AddressPoolRepo) PopulateFreelistForPool(ctx context.Context, poolID string) error {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return fmt.Errorf("populate freelist: pool %s: %w", poolID, ErrNotFound)
		}
