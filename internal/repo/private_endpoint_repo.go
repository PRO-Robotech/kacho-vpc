...
// PrivateEndpointRepo — реализация ports.PrivateEndpointRepo поверх pgxpool.
...
func (r *PrivateEndpointRepo) List(ctx context.Context, f ports.PrivateEndpointFilter, p ports.Pagination) ([]*PrivateEndpoint, string, error) {
...
func (r *PrivateEndpointRepo) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		if isFKViolation(err) {
			return fmt.Errorf("%w: private endpoint is in use", ports.ErrFailedPrecondition)
		}
		return wrapPgErr(err, "PrivateEndpoint", id)
	}
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("%w: PrivateEndpoint %s not found", ports.ErrNotFound, id)
	}
