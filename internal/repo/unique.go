...
func wrapPgErr(err error, kind, id string) error {
	...
	if isInvalidUUID(err) {
		...
		return fmt.Errorf("%w: invalid %s id '%s'", ports.ErrInvalidArg, ycKindText(kind), id)
	}
	if isFKViolation(err) {
		return fmt.Errorf("%w: %s has dependent resources", ports.ErrFailedPrecondition, kind)
	}
	if isCheckViolation(err) {
		...
		return fmt.Errorf("%w: %s violates check constraint", ports.ErrInvalidArg, kind)
	}
	if isExclusionViolation(err) {
		return fmt.Errorf("%w: value conflicts with existing %s", ports.ErrInvalidArg, kind)
	}
