...
//   16       — Delete подсети снимает её блоки (FK ON DELETE CASCADE).
...
func TestIntegration_SecondaryCidrOverlap_CrossSubnet_PrimaryHit(t *testing.T) {
	...
	// sub-1 не изменён — secondary не добавлен.
	rd, err := r.Reader(ctx)
	require.NoError(t, err)
...
func TestIntegration_SecondaryCidrOverlap_CrossNetwork_OK(t *testing.T) {
	...
	// sub-B1 (net-B) берёт 10.0.0.0/24 — занят sub-1, но в ДРУГОЙ сети → OK.
...
func TestIntegration_SecondaryCidrOverlap_RemoveFreesRange(t *testing.T) {
	...
	// Remove 10.0.5.0/24 с sub-1: SetCidrBlocks тем же путём (remaining-набор).
	w, err := r.Writer(ctx)
	require.NoError(t, err)
	...
	// Теперь sub-2 успешно занимает освобождённый 10.0.5.0/24.
...
// её блоки из инварианта (FK ON DELETE CASCADE), диапазон занимаем заново
// (acceptance 16).
func TestIntegration_SecondaryCidrOverlap_DeleteFreesRange(t *testing.T) {
	...
	// Новая подсеть берёт оба освобождённых диапазона.
