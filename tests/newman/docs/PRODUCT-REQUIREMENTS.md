...
Gateway / PrivateEndpoint; все folder-scoped (`project_id` обязателен в Create); все поддерживают
...
`Move` ресурса в его же `project_id` → sync `InvalidArgument "Illegal argument Destination folder
...
### REQ-RES-07 — SecurityGroup: `network_id` ОБЯЗАТЕЛЕН + НЕИЗМЕНЯЕМ (KAC-243)      [P1]
> **KAC-243 реверт `kacho-proto#8`.** Раньше `network_id` был опционален (SG без сети допустима) —
...
Operation НЕ создаётся); well-formed но несуществующий → sync `NOT_FOUND` `Network … not found`.
...
- Agent-check: `kacho-proto/.../security_group_service.proto` (`CreateSecurityGroupRequest.network_id`
...
- Agent-check: `internal/migrations/0001_initial.sql` (`networks_project_id_name_key`) + `0002_resource_name_unique.sql`; `mapRepoErr` (`23505` → `ErrAlreadyExists`).
...
200, pool создаётся как v4-only. Free-list `address_pool_free_ips` материализуется
для каждого `/N` блока (`2^(32-N)-2` строк). `ipv6_pool_cursors` для v4-only pool
не инициализируется.
- Validated-by: `IPL-CR-CRUD-V4-OK`
- Agent-check: `kacho-proto/.../internal_address_pool_service.proto` (`AddressPool.v4_cidr_blocks=13`); `internal/service/address_pool_service.go::Create`.

### REQ-IPL-CR-02 — Create v6-only AddressPool                                  [P0]
`Create` с `v4_cidr_blocks=[]` и `v6_cidr_blocks` непустым → 200, pool создаётся как
v6-only. `ipv6_pool_cursors` инициализируется (`InitIPv6PoolCursor`, sparse counter).
- Validated-by: `IPL-CR-CRUD-V6-OK`
- Agent-check: `internal/service/address_pool_service.go::Create`; `internal/repo/address_pool_repo.go` (`InitIPv6PoolCursor` для v6-блоков).
...
- Agent-check: `internal/service/address_pool_service.go::Create` — оба пути материализации.
...
both empty". Pool не создаётся / не обновляется. Defensive backstop на DB-уровне —
PG `RAISE EXCEPTION` в backfill миграции `0022` (REQ-MIG-06, см. ниже).
- Validated-by: `IPL-CR-VAL-BOTH-EMPTY`, `IPL-UPD-EMPTY-BOTH-REPLACE`
- Agent-check: `internal/service/address_pool_service.go` sync-валидация в начале `Create`/`Update`.
...
- Agent-check: `internal/service/address_pool_service.go` — per-slot validate family.

### REQ-IPL-UPD-01 — Update full-replace `v4_cidr_blocks` с явным флагом         [P0]
`Update` с `replace_v4_cidr_blocks=true` + `v4_cidr_blocks=[нов]` → full-replace v4-блоков
(старые удалены, новые INSERTed); `v6_cidr_blocks` НЕ трогается (флаг не выставлен).
`address_pool_free_ips` пере-материализуется только для нового v4-диапазона.
- Validated-by: `IPL-UPD-REPLACE-V4`
- Agent-check: `internal/service/address_pool_service.go::Update` — handle `replace_v4_cidr_blocks` отдельно от тела.

### REQ-IPL-UPD-02 — Update full-replace `v6_cidr_blocks` с явным флагом         [P0]
Симметрично REQ-IPL-UPD-01: `replace_v6_cidr_blocks=true` + `v6_cidr_blocks=[нов]` →
full-replace v6-блоков; v4-часть pool без изменений; `ipv6_pool_cursors` сбрасывается
на новый CIDR.
- Validated-by: `IPL-UPD-REPLACE-V6`
- Agent-check: `internal/service/address_pool_service.go::Update`; `internal/repo/...` resetIPv6Cursor.

### REQ-IPL-UPD-03 — Update post-state с обоими family-списками пустыми → InvalidArgument [P0]
Если после применения replace-флагов и body итоговое состояние pool — оба `v4_cidr_blocks=[]`
и `v6_cidr_blocks=[]` → sync `InvalidArgument` "must not be both empty after update".
Покрывает `replace_v4=true,v4=[],replace_v6=true,v6=[]` (dual-stack→empty) и попытку
очистить единственный family (v4-only `replace_v4=true,v4=[]` без `replace_v6`).
- Validated-by: `IPL-UPD-EMPTY-BOTH-REPLACE`
- Agent-check: `internal/service/address_pool_service.go::Update` — post-state guard.

### REQ-IPL-UPD-05 — Update: очистить один family на dual-stack pool             [P1]
`replace_v6_cidr_blocks=true, v6_cidr_blocks=[]` на dual-stack pool → 200, pool становится
v4-only (v4-часть preserved, v6 cleared). Симметрично для clear v4 на dual-stack с
preserved v6. Invariant `v4 ∪ v6 ≠ ∅` соблюдён.
- Validated-by: `IPL-UPD-CLEAR-V6-DUALSTACK-TO-V4-ONLY`
- Agent-check: `internal/service/address_pool_service.go::Update`.

### REQ-IPL-UPD-06 — Update без replace-флагов → CIDR-поля игнорируются           [P1]
`Update` без `replace_v4_cidr_blocks` И без `replace_v6_cidr_blocks` (либо оба false) →
body-значения `v4_cidr_blocks`/`v6_cidr_blocks` игнорируются (даже если непустые); v4 и v6
части pool остаются как были. Mutable не-CIDR поля (`description`, `labels`, `is_default`)
применяются как обычно.
- Validated-by: `IPL-UPD-NO-FLAGS-NOOP`
- Agent-check: `internal/service/address_pool_service.go::Update` — флаг как единственный триггер replace.
...
- Agent-check: `internal/service/network_service.go::BindAsNetworkDefault` + аналогичные RPC — НЕТ family-check; family-фильтр только в `doResolve`.
...
→ gRPC `FailedPrecondition` (код 9, не `Internal` 13). После KAC-71 фильтр работает через
...
- Agent-check: `internal/service/address_pool_service.go::doResolve` — `poolHasFamily` заменён на `len(V6CIDRBlocks)>0` для FamilyV6.
...
- Agent-check: `internal/service/address_pool_service.go::doResolve` — `len(V4CIDRBlocks)>0` для FamilyV4.
...
- Agent-check: `internal/handler/internal_address_pool_handler.go::ExplainResolution`.
...
- Agent-check: `internal/service/address_pool_service.go::doResolve` — Step 1 (address_pool_address_override) применяет тот же family-filter.
...
- Agent-check: `internal/service/address_pool_service.go::doResolve` — Step 2 (address_pool_network_default) с тем же family-filter.
...
- Agent-check: `internal/repo/*.go` `List` — `WHERE project_id = $1`.
...
### REQ-SG-RULE-SAME-NETWORK — SG-target rule только в пределах той же Network (KAC-243) [P1]
SG-target rule (`oneof target = security_group_id`) разрешён **только** если target-SG в той же
...
Sync fast-fail (Operation НЕ создаётся). Применяется на `Create` (`rule_specs`), `UpdateRules`
(`addition_rule_specs`), `UpdateRule` (смена target). CIDR-rule / `predefined_target` — не затронуты.
Same-network target → OK. Service-layer-валидация (acceptance D3 — достаточна для KAC-243; без
нормализованной rule-target-таблицы).
...
- Agent-check: `internal/apps/kacho/api/securitygroup/{create,update_rules,update_rule}.go` — same-network
  reader + `BadRequest.field_violations`; acceptance `sub-phase-securitygroup-network-mandatory-and-same-network-rules-acceptance.md` §C / D1.

### REQ-SG-MOVE-NETWORK-BOUND — Move network-bound SG между проектами запрещён (KAC-243) [P1]
...
`checkMoveDestination` (same-folder check) и до создания Operation. Обоснование: `network_id`
...
- Validated-by: `SG-NET-19-NEG-MOVE-FORBIDDEN` (cross-project), `SG-MV-CRUD-OK` (same-project —
  guard precedence над same-folder check).
- Agent-check: `internal/apps/kacho/api/securitygroup/move.go` — `if cur.NetworkID != "" { … FailedPrecondition }`
  до `checkMoveDestination`; acceptance §G / D5.
...
### REQ-MOVE-01 — Move в другой folder обновляет project_id                        [P1]
`Move(destination_project_id)` существующего ресурса → `project_id` обновлён; ресурс виден в `List` нового folder.
...
`NetworkInterface` — публичный folder-scoped ресурс (`project_id` обязателен), принадлежит `Subnet`
...
### REQ-NIC-05 — NIC несёт `security_group_ids[]`                                  [P2]
`NetworkInterface` несёт `security_group_ids[]` — ссылки на существующие SG. `Create` NIC с такой SG → 200.
(KAC-243: SG теперь mandatory-network — REQ-RES-07; кейс создаёт SG, привязанную к сети NIC'а. NIC сам
network-bound валидацию SG-привязки не навязывает — проверяет лишь существование SG.)
- Validated-by: `NIC-CR-WITH-UNBOUND-SG-OK`
- Agent-check: `internal/service/network_interface.go` Create/Update — валидация существования SG.

### REQ-NIC-06 — публичная проекция NIC — lean (без инфра-полей)                  [P0]
Публичная `NetworkInterface` (`Get`/`List`/`Create`-result) содержит ТОЛЬКО `id`/`project_id`/`name`/
