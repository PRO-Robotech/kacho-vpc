...
| `SG-NET-01-NEG-CREATE-NO-NETWORK` | VAL,NEG | P0 | 1 (sec) | KAC-243: Create SecurityGroup без `network_id` → sync 400 INVALID_ARGUMENT verbatim `network_id required` (Operation НЕ создаётся; network_id mandatory, реверт kacho-proto#8). Verifies REQ-RES-07. |
| `SG-NET-02-CREATE-OK` | CRUD | P0 | 1 (sec) | KAC-243 happy: Create SecurityGroup с валидным `network_id` → Operation done без error → GET echoes `networkId`. Verifies REQ-RES-07. |
| `SG-NET-03-NEG-NETWORK-NOTFOUND` | CONF,NEG | P1 | 1 (sec) | KAC-243: Create SG с well-formed но несуществующим `network_id` (`enp00…`) → sync 404 NOT_FOUND `Network … not found` (fast-fail, Operation НЕ создаётся). Verifies REQ-RES-07. |
| `SG-NET-04-NEG-UPDATE-MASK-NETWORK` | STATE,VAL,NEG | P1 | 1 (sec) | KAC-243: Update реальной SG с `update_mask=network_id` → sync 400 INVALID_ARGUMENT (network_id не в known-mask = immutable); GET → networkId неизменён. Verifies REQ-RES-07. |
| `SG-NET-07-NEG-RULE-CROSS-NETWORK-CREATE` | VAL,NEG,CONF | P1 | 1 (sec) | KAC-243: Create SG в net-A с SG-target rule на SG из net-B → 400 INVALID_ARGUMENT `security group rule can only reference a security group in the same network` + `BadRequest.field_violations[].field = rule_specs[0].security_group_id`. Verifies REQ-SG-RULE-SAME-NETWORK. |
| `SG-NET-08-RULE-SAME-NETWORK-OK` | CRUD,STATE | P1 | 1 (sec) | KAC-243 happy: Create SG в net-A с SG-target rule на SG той же net-A → Operation done; rule сохранён (`securityGroupId` виден в GET). Verifies REQ-SG-RULE-SAME-NETWORK. |
| `SG-NET-09-NEG-RULE-CROSS-NETWORK-UPDATERULES` | VAL,NEG,STATE | P1 | 1 (sec) | KAC-243: UpdateRules на SG(net-A), добавляющий SG-target rule на SG из net-B → 400 INVALID_ARGUMENT same-network + `field_violations[].field = addition_rule_specs[0].security_group_id`; набор правил не изменён. Verifies REQ-SG-RULE-SAME-NETWORK. |
| `SG-NET-09-RULE-SAME-NETWORK-UPDATERULES-OK` | CRUD,STATE | P1 | 1 (sec) | KAC-243 happy: UpdateRules на SG(net-A), добавляющий SG-target rule на SG той же net-A → Operation done; rule виден в GET. Verifies REQ-SG-RULE-SAME-NETWORK. |
| `SG-NET-19-NEG-MOVE-FORBIDDEN` | NEG,CONF,STATE | P1 | 1 (sec) | KAC-243: Move network-bound SG в другой проект → sync 400 FAILED_PRECONDITION `security group cannot be moved between projects while bound to a network`; SG неизменна (projectId/networkId). Verifies REQ-SG-MOVE-NETWORK-BOUND. |
...
| `NIC-CR-WITH-UNBOUND-SG-OK` | CRUD | P2 | 1 (nic) | Create NIC с `security_group_ids[]` на SG (bound к сети NIC'а) → 200; NIC ссылается на SG. (KAC-243: SG теперь mandatory-network — network-less SG больше нет; SG привязан к `{{netId}}`.) Verifies REQ-NIC-05. |
...
| `SG-LIST-FILTER-NETWORK-OK` | CRUD,FILTER | P2 | 1 (sec) | List SecurityGroups с фильтром по `network_id` → возвращает только SG net-A; SG из другой сети net-B отсутствует. (KAC-243: negative-сторона = SG из другой сети, т.к. network-less SG больше нет.) Verifies REQ-RES-07. |
...
| `*-MV-CRUD-OK` | CRUD | P1 | 5 (add,gat,net,rou,sub) | Move subnet в другой folder → project_id обновлён. (KAC-243: SG-инстанс переписан как NEG guard — см. `SG-MV-CRUD-OK` под `# index: SG-NET-19-NEG-MOVE-FORBIDDEN`; happy Move для network-bound SG невозможен.) |
| `*-MV-IDM-SAME-FOLDER` | CRUD,IDM | P2 | 5 (add,gat,net,rou,sub) | Move в текущий folder → ok (idempotent), ресурс остаётся. (KAC-243: SG-инстанс удалён — network-bound Move-guard бьёт раньше same-folder check; см. `SG-MV-CRUD-OK`.) |
