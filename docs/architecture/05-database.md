...
| 0002 | `0002_resource_name_unique.sql` | partial UNIQUE `(folder_id, name) WHERE name <> ''` для `subnets`/`route_tables`/`security_groups`/`gateways`/`private_endpoints`/`addresses` (закрыл расхождение с verbatim YC — раньше UNIQUE был только у Network; см. FINDING-005 в `tests/newman/docs/BUG-MAP.md`) |
