...
| golang-cli | n/a | — | `kachoctl-ipam` CLI **удалён** (admin-операции — curl/REST на api-gateway internal mux / web-UI). Раньше: minimal `flag.NewFlagSet`, subcmd dispatch, shared `--addr`. |
| golang-observability | ⚠️ | group-5 audit | slog с json уже стандарт ✓. **Gap** (GitHub Issue — observability gap): нет Prometheus metrics; нет OpenTelemetry (для distributed trace cascade resolve). |
...
| golang-troubleshooting | ⏭️ | not done | pprof endpoint не подключен. Можно через `net/http/pprof` blank-import + отдельный mux. **GitHub Issue — observability gap**: добавить вместе с metrics. |
...
| golang-documentation | ✅ | already done | docs/architecture/ файлы (00-09 + 07-known-divergences) + roles/skills + это (09). godoc на public types через CLAUDE.md "default no comments" — есть только critical-WHY. README.md + CLAUDE.md. |
...
| golang-spf13-cobra | У сервиса нет CLI (`kachoctl-ipam` удалён) — cobra не нужен |
...
- ⚠️ **3 gap'а** заведены как GitHub Issues: observability (Prom/OTel), security middleware, integration test runs в CI.
