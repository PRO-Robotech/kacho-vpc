...
| Admin-инструменты (`kachoctl-ipam`, UI) | gRPC `:9091` через api-gateway internal listener | Управление Region/Zone/AddressPool |
...
|  +------------------------+    | kacho-vpc-controllers |  |
|                                | + kacho-compute       |  |
|                                +-----------------------+  |
+-----------------------------------------------------------+
```

API-gateway тянет публичный сервис; internal listener доступен только
внутри кластера и закрывается NetworkPolicy.
...
| `KACHO_VPC_DB_MAX_CONNS` | `0` (pgx default) | Размер pool'а |
...
- `internal/migrations/0001_initial.sql` — squashed схема, embedded через
  `embed.FS` (объявлено в `migrations.go`).
...
В `cmd/kachoctl-ipam` лежит отдельный CLI для admin-операций над IPAM
(работает по gRPC через api-gateway internal listener).
...
|---|---|---|---|
| Network | `enp` | `default_security_group_id`, `route_distinguisher` | default SG создаётся inline в `doCreate` |
...
| `zones_region_id_name_key` | UNIQUE `(region_id, name)` | Имя зоны уникально в регионе |
...
Все миграции в `0001_initial.sql` (squashed). При первом запуске goose
создаёт `goose_db_version` автоматически.
...
4. Описать `migrations.go` с `//go:embed *.sql` для `embed.FS`.
...
kacho-vpc/
├── cmd/
│   ├── vpc/main.go                — composition root (gRPC servers + wiring)
│   └── kachoctl-ipam/             — admin CLI для IPAM
├── internal/
│   ├── config/config.go           — envconfig, Config + DSN()
...
│   └── migrations/
│       ├── 0001_initial.sql       — squashed schema
