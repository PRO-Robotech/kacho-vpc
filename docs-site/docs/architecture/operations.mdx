...
flowchart LR
  ...
  notify --> watch["InternalWatchService<br/>(dedicated pgx.Conn · LISTEN)"]
  watch -->|"Event{seq,type,id,op,payload}"| peer["peer-сервис<br/>(admin-tooling / UI)"]
```

После `COMMIT` триггер `vpc_outbox_notify_trg` (AFTER INSERT) шлет `pg_notify('vpc_outbox', sequence_no)`.
`InternalWatchService` слушает этот канал на **выделенном** соединении (`pgx.Connect`, вне пула —
`LISTEN` некорректен на pooled-conn), делает catch-up по `sequence_no` и стримит события подписчику.

:::info Outbox — для server-to-server, не для клиента
Поток `vpc_outbox` через `InternalWatchService` — **внутренний** (порт `:9091`), для peer-сервисов
и admin-tooling. **Tenant-клиенты его не используют** —
они **поллят** `OperationService.Get` (Watch RPC в API Kachō не существует). Подробнее об outbox-стриме —
[Архитектура](/architecture/overview).
:::

:::note Второй outbox — регистрация owner-tuple в FGA
Помимо доменного `vpc_outbox` (Watch/realtime) worker на Create/Delete пишет — **в той же
