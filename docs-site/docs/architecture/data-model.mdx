...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>vpc\_outbox</code></td><td>Доменный транзакционный outbox + <code>pg\_notify</code> (Watch)</td><td><code>sequence\_no</code></td><td>—</td></tr>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>vpc\_outbox</code></td><td>Транзакционный outbox: каждая успешная мутация пишет событие в outbox <strong>в той же TX</strong>, что и сам ресурс. Триггер <code>vpc\_outbox\_notify\_trg</code> делает <code>pg\_notify('vpc\_outbox', sequence\_no::text)</code>; <code>InternalWatchService</code> читает через <code>LISTEN vpc\_outbox</code> на dedicated-соединении.</td></tr>
  </tbody>
</table>
...
flowchart LR
  ...
  ob -->|"trigger pg_notify"| watch["InternalWatchService<br/>LISTEN vpc_outbox"]
