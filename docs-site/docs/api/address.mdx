...
Адрес принадлежит проекту (`project_id`). Адрес, привязанный к сетевому интерфейсу
(`used = true`), **нельзя удалить** — сначала отвяжите его от `NetworkInterface`. Спецификация вида
адреса (`external_*` / `internal_*`) и `project_id` — **immutable** после `Create`.
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>used</code></td><td><code>{TYPES.bool}</code></td><td>Адрес используется (output-only; <code>true</code>, когда привязан к <code>NetworkInterface</code>)</td></tr>
...
**Адрес в использовании удалить нельзя:** если `used = true` (привязан к `NetworkInterface`) —
`FAILED_PRECONDITION`; сначала отвяжите адрес от NIC (убрать его id из `v4_address_ids` /
`v6_address_ids` через `NetworkInterface.Update`). Также удаление блокируется при включенной
`deletionProtection` (`FailedPrecondition`).
...
`Address` — независимый ресурс: он создается, существует и удаляется отдельно от
`NetworkInterface`. Связь устанавливается со стороны интерфейса — id адреса добавляется в
`v4_address_ids` / `v6_address_ids` интерфейса (см. [NetworkInterface](/api/network-interface)).
Как только адрес попадает в этот список, сервис помечает его `used = true`, и адрес становится
...
- **Удаление используемого адреса** (`used = true`) → `FailedPrecondition`. Сначала уберите id
  адреса из `v4_address_ids` / `v6_address_ids` интерфейса (`NetworkInterface.Update`), затем
  удаляйте. То же для `deletionProtection = true` — снимите защиту перед удалением.
