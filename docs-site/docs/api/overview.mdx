...
`Internal*`-сервисы (IPAM, `AddressPool`, `InternalWatchService`, …) живут на cluster-internal
listener (`:9091`) и не публикуются на external TLS endpoint. Они предназначены для
peer-сервисов / admin-UI, см. [Авторизация и приватность](/architecture/authz).
:::
...
жирным) возвращают `Operation`. `Internal*`-сервисы (IPAM, `AddressPool`, `InternalWatchService`
и пр.) сюда не входят — они доступны только на cluster-internal listener.
